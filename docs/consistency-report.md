...
| Checkout blocking is not payment-authority failure | payment normalization, basis, and tests |
| Generated claims are scoped capabilities | generated-claim module and example |
| Generated-claim status has one canonical precedence | normalizer, semantic evaluator, schema, and tests |
...
The runtime modules consistently enforce strict date normalization, evidence identity and explicit content hashes, freshness dependency identity and conservative horizon merging, protected decision identity, action-aware result precedence, payment non-evaluation before actor-authority and checkout prerequisites, semantic section coherence, token-based reason classification, canonical generated-claim precedence, axis coherence, required allowed-use scope, case-sensitive opaque identities, verified direct parent-projection binding, deterministic dependency ordering, causal multi-hop refusal propagation, explicit lineage overflow, Ed25519 metadata and key validation, hash recomputation, exact reason/component reconciliation, and live request rebinding at verified external projection.
...
`npm run validate:shape` traverses the complete canonical JSON Schema without external dependencies. It validates unsigned, HMAC, Ed25519, and committed Travel Backpack envelopes; rejects unknown fields, malformed canonical timestamps, malformed Ed25519 values, incoherent payment sections, allowed claims without a declared use, and claim axes whose blockers disagree with their status; and tests schema action, surface, actor, eligibility-source, result, authority, payment-authority, next-safe-action owner, generated-claim status, and generated-claim axis vocabularies against the runtime constants.
