...
`projectTrustedAgentCommerceDecisionEnvelope` verifies the target surface, authenticator policy, trusted key identity, complete envelope integrity, and freshness before projection. For feed, tool, checkout, and protocol output, it also requires an `expectedRequest` containing the live requested action, subject, and actor and rejects any mismatch with the protected envelope.
